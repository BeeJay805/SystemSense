...
| Broad Windows coverage | 15 registered probes: baseline topology/configuration, events, listeners, resource pressure and GPU telemetry | Implemented; unsupported and omitted sources remain explicit |
...
| Shared reference knowledge | 66 nodes, 102 conditional relations, 18 primary sources; explicit runtime Windows error lookup | Implemented separately from machine evidence |
...
| Repair boundary | Explicit consent, target/precondition binding, journal, rollback limits, independent outcome verification | Typed proposals and authorization gate implemented; concrete executor and outcome verification remain |
