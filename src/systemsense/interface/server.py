...
async function initialize() {{
  try {{
    ...
    const availability = inference.enabled === false ? " Model inference is off; deterministic evidence checks remain available." : " Configured local models are advisory and not diagnostically qualified.";
    const providers = [inference.decision_model, inference.reasoning_model].filter(Boolean);
