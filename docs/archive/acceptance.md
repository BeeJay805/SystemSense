...
| Broad Windows coverage | 17 registered probes: baseline topology/configuration, staged passive connectivity, events, listeners, resource pressure, GPU telemetry, and calling-desktop display mode | Implemented; unsupported and omitted sources remain explicit |
...
| Local advisory inference | Managed schema-v3 CUDA Laya with exact worker lease, fresh telemetry and deterministic reasoning; historical pinned Qwen3.8 27B local research | Legacy GPU profiles degrade deterministically; managed Qwen overlap and general diagnostic quality remain unqualified |
| Shared reference knowledge | 95 nodes, 136 conditional relations, 35 source entries; explicit runtime Windows error lookup | Implemented separately from machine evidence; game frame-time, other-monitor/dynamic-refresh and per-game GPU-engine observations remain absent |
