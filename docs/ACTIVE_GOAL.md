...
| Self-contained semantic evidence packets | Versioned per-fact serializer with exact values or explicit truncation, timing, status, omissions and grounded relations. Mixed measurement turns now freeze all mandatory candidate sources plus a bounded, revalidated focused-context slice, rather than hiding non-candidate observations behind the receipt. Optional rows pass the same projector used by strict freeze. | Contract and synthetic v2 loader tests passed; actual captured-corpus parity pending. The eight-row attention target records omitted optional rows in case warnings, not yet in a model-visible omission contract. |
...
The focused-context receipt increment keeps both mandatory measurement
sources and up to the bounded attention target of projectable current-case
context in the exact frozen model request. Optional rows are preflighted
through the receipt repository's projector; missing or cross-case claimed
current context closes a gap, while malformed same-case optional rows and
capacity omissions become distinct case warnings. Strict final freeze and
measurement admission remain unchanged. Astra's independent review found and
verified closure of the malformed-optional-row failure; its broader review
confirmed the host/model architecture while identifying managed Qwen overlap
as still unadmitted. This exact revision passed **2,825 tests, 23 explicit
opt-in skips, and seven expected warning-path warnings**, strict Pyright,
Ruff lint/format, offline source and wheel build, and Git whitespace checks.
...
investigator loop iteration, preserves an unresolved page across later turns,
and records a gap for failed policy, stale focus, terminal references, deadline,
source disappearance, or insufficient capacity. A vanished historical pending
row remains an unresolved reference and cannot be delivered as a success. An
exact stored observation is marked satisfied only
when it enters the focused context and the checkpoint, turn outcome, item
...
Schema 31 addresses a narrow liveness limit exposed by a prior ordinary-run
regression. When failed read-only probe coverage advances the same case's
evidence generation while deep reasoning is in flight, a later event turn can
atomically reread the pending sources, replace old requested IDs with
current-generation IDs, and continue focused delivery. The original event and
old IDs remain historical; the old IDs become obsolete, not satisfied. The
coordinator refreshes its focused packet and exact-ID metadata before the
turn, and a retained tail drains before catalog discovery restarts at the head.
Missing sources, insufficient item capacity, and changed versions retain an
explicit gap. Integration tests prove the ordinary failed-probe/deep overlap,
FIFO continuation, cursor restart, and capacity fallback; they are synthetic
contract checks, not measured diagnosis, useful Laya routing, or permission
to train or repair.

The schema-31 integrated gate passed **2,788 tests, 23 explicit opt-in skips,
and seven expected warning-path warnings**. Strict Pyright, Ruff lint and
format, and the offline source/wheel build passed. Storage tests include a
genuine populated schema-30 upgrade, repeated two-item refresh across two
generation advances, capacity rollback, stale-generation rejection, and v1
readback. Astra independently reviewed the storage/coordinator boundary and
found no remaining confirmed P1/P2 defect in that bounded review. No
qualified schema-31 latency, held-out diagnostic utility, ordinary-laptop
performance, or controlled repair/recovery result follows from this gate.

One opt-in pinned-Laya refresh smoke subsequently delivered two synthetic
stored observations across a same-case generation advance. The first turn
left one pending reference; the second used real Laya, marked the old ID
obsolete, and satisfied its current-generation successor in the focused
checkpoint. The single warm post-append-to-checkpoint observation was 219.8 ms,
after model prewarm and under non-isolated GPU conditions. The raw record is
`%LOCALAPPDATA%/Temp/systemsense-schema31-refresh-20260924-6.json`; it binds
the harness source hash, model/profile hashes, base production revision, GPU identity,
and exact event/turn/item IDs, with prewarm and resource waits recorded
separately. This is a narrow model-path integration smoke with
...
format, the offline source/wheel build, and Git whitespace checks passed.
Integration regressions cover FIFO continuation, generation/deadline races,
source loss during ranking, checkpoint save, stop, expiry, and owner recovery,
plus a vanished non-source pending row. Astra reviewed the mounted architecture
and exposed the combined source-loss and pending-row failures before their
corrections. This gate establishes custody and bounded orchestration, not
held-out diagnosis, real-Laya latency, or safe autonomous repair.
...
The schema-29 dormant investigator-session checkpoint and general retrieval
delivery correction passed **2,704 tests**, with 20 explicit opt-in skips, one
optional MCP deselection, and seven expected warning-path warnings. Strict
Pyright, Ruff lint/format, offline source/wheel build, and Git whitespace
checks passed. The session tests cover independent event custody, active-only
backpressure, restart, source loss, staleness, deadlines, clock rollback, and
tamper readback. Astra independently found and rechecked deadline, source-loss,
and chronology guards. No new event-driven runtime mount, real-Laya latency
sample, controlled diagnosis, or training-admissible label follows from this.
...
cross-process fast-model turns. An in-process fair turn gate now admits one
advisory callback at a time, then yields to other waiting cases before another
turn. At case stop the owner closes its offer queue and records explicit
exact-parent gaps for unconsumed or late work; a live callback keeps its lease
until it really exits. Astra's follow-up review found deadline and multi-offer
...
passed 2,575 tests, 20 skips and six expected warnings. Two earlier full-suite
runs had the same intermittent
...
scoped integration review found no premature-release or crash-window defect
in the cooperating default case path after the ledger hot path was bounded;
passive integration was added afterward and covered by the full suite, not
that review. Strict Pyright, Ruff lint/format, offline source/wheel build, and
...
build, and Git whitespace checks passed. A read-only live connectivity snapshot
showed one available WLAN interface with no omitted WLAN rows; it was not a
fault episode or diagnosis. The committed-event real-Laya latency attempt
stopped at its resource gate because unrelated GPU activity could not be
ruled out, producing zero model timing samples. The schema-26 tests prove
one-shot claim, actual runtime execution linkage, evaluated/unknown/failed
...
The previous continuation added per-WLAN passive stage correlation with exact GUID and
...
This pass adds a schema-2 local profile selecting a deterministic CPU
typed-feature router with the pinned local reasoner; it does not promote that
...
The preceding non-MCP suite passed 1,219 tests with 15 gated skips and one MCP
...
A new CPU-only two-preview/four-probe Laya smoke on this desktop took 25.125 s
on the first worker call and 10.157 s warm. On the same synthetic
54-preview/17-capability request hash, the typed-feature router timed 0.4342 ms
warm p95 and keyword routing 0.0755 ms warm p95. These measure provider latency,
not useful-probe choice or ordinary-laptop suitability.
...
4. Prove one exact WinINet repair through owned-route evidence, same-user
...
5. Measure local fast-brain candidates on ordinary Windows laptops. Require
   useful-probe quality and resource/latency floors; train only on admitted
   reviewed labels with grouped held-out tests.
6. Expand Wi-Fi and gaming with separate physical rigs and workload oracles.
