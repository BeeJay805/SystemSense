...
def test_service_fingerprint_excludes_volatile_runtime_state() -> None:
    ...
    assert '"$serviceName|$serviceStartMode|$($_.PathName)"' in service_inventory
    assert "$_.StartMode" in service_inventory
...
def test_service_fingerprint_normalizes_only_per_user_instance_suffixes() -> None:
    ...
    assert '$serviceStartMode = if ($_.Name -eq "BITS")' in service_inventory
    assert '"<trigger-managed>"' in service_inventory
    assert '"$serviceName|$serviceStartMode|$($_.PathName)"' in service_inventory
