...
| Bounded frontier and retrieval beyond model packet | Schema-21 lifecycle, fair catalog/graph seeding, live source-bound retrieval with a focused-delivery terminal gate, exact graph-branch delivery, and durable deep-question mailbox. One pre-target PDF choice ranks stored retrievals beside a process measurement with a frozen, source-authenticated receipt. Schema-29 adds a separate, dormant investigator-event custody and bounded reconsideration-session foundation. | The new session API is **not mounted**: turn consumption, cursor continuation, current owner/context revisions, typed backpressure, and restart-safe policy reconciliation remain missing. The PDF choice is one bounded step, not a persistent mixed policy across resumed rounds; general measurement ranking and independent useful-search gain remain missing |
