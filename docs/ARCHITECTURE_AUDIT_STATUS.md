...
| F03 cross-batch prerequisites | Partial | [Eligibility](../src/systemsense/application/investigator.py) checks durable satisfied probe executions before closing the dependency chain; contract tests cover prior success, but a persisted cross-batch success/invalidation regression remains. |
...
| F09 diagnostic progress | Open | [Typed progress rules](../src/systemsense/evaluation/progress.py) are not mounted in the live stop decision; raw novelty remains insufficient. |
...
| F24 descendant containment | Open | [Collector executor](../src/systemsense/orchestration/executor.py) still needs a qualified Windows process-tree containment test and contract. |
