...
Status: proposed, independently reviewed; **not mounted in the ordinary event
loop**. The current schema-30/31 event consumer retrieves stored evidence only.
The existing mixed PDF route is a narrow canary, not a general persistent
measurement policy. This note defines the next implementation boundary without
claiming that a ranked measurement has run.
...
The checkpoint that records `measurement_admitted` must commit the exact
ranking snapshot, selected frontier item, admission, worker-claim custody, and
turn outcome together, or an equivalent fenced continuation must prove the
same identity. A proposed first implementation claims a one-shot dispatch in
that short transaction *before* advancing the checkpoint, then launches the
read-only worker afterward. This requires a **new, versioned, single-use launch
continuation API**. The current worker rejects a changed case epoch and a
previously claimed admission; simply moving the existing claim call is not a
working or safe implementation. The continuation must bind the old selection
and admission epoch, exact resulting checkpoint, owner, task, invocation, and
claim. Its verifier accepts only that one authorized checkpoint transition,
not arbitrary later state changes.
...
The first mixed turn should offer at least one eligible retrieval and one
eligible registered measurement for a non-PDF case. Later turns must preserve
unselected alternatives without carrying stale candidate authority. The
current FIFO retrieval tail is correct for historical v1/v2 records, but a
v3 mixed turn needs bounded reranking of current references. Persist offered
order, deferral counts, and an aging limit so a long stored catalog cannot
starve measurements or vice versa. Fairness guarantees that work is offered;
it does not guarantee Laya chooses useful work.
